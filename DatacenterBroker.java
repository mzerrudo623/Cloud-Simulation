...
public class DatacenterBroker extends SimEntity {
	...
	//value is 1 for best fit, 2 for worse fit, 0 for first fit
	...
	protected void submitCloudlets() {
		...
		//if algorithm is 1, use best fit
		//algorithm is 2, use worse fit
		//anything else is first fit
		if (algorithm == 1){
			bestFitAlgorithm();
		}
		else if (algorithm == 2){
	...
	public void setAlgorithm(int num){
		if (num >= 3 || num <= -1){
			System.out.println("Value of algorithm must be 0, 1, or 2");
		}
		else{
			algorithm = num;
		}
	...
	public void bestFitAlgorithm(){
		//index of the best vm
		int bestIndex = 0;
		
		//for each cloudlet in the list
		for (int i = 0; i < cloudletList.size(); i++){
			//for each vm in the list
			for (int j = 1; j < vmsCreatedList.size(); j++){
				//if this vm has more mips than the current best, make the index of this vm the new best
				if ((int)vmsCreatedList.get(j).getMips() > (int)vmsCreatedList.get(bestIndex).getMips()){
					bestIndex = j;
				}
			}
			//assign the vm in best index to the current cloudlet
			cloudletList.get(i).setVmId(vmsCreatedList.get(bestIndex).getId());
			//set the current allocated mips of the vm
			vmsCreatedList.get(bestIndex).setCurrentAllocatedMips(cloudletList.get(i).getCloudletLength());
			//reset best index to 0
			bestIndex = 0;
		}
	}
	
	//algorithm for worse fit
	public void worseFitAlgorithm(){
		//index of the best vm
		int bestIndex = 0;
		...
		for (int i = 0; i < cloudletList.size(); i++){
			//for each vm in the list
			for (int j = 1; j < vmsCreatedList.size(); j++){
				//if this vm has less mips than the current best, make the index of this vm the new best
				if ((int)vmsCreatedList.get(j).getMips() < (int)vmsCreatedList.get(bestIndex).getMips()){
					bestIndex = j;
				}
			}
